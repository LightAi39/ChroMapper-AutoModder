...
namespace ChroMapper_LightModding.UI
{
    internal class SongInfoUI
    {
        ...
        public void AddInfoMenu(Transform parent)
        {
            ...
            UIHelper.AddButton(_infoMenu.transform, "ViewSongInfoComments", "View Song Info Comments", new Vector2(36, -45), () =>
            {
                ShowAllSongInfoCommentsMainUI();
            }, 64, 25, 10);

            UIHelper.AddButton(_infoMenu.transform, "ExportAMDiscordSeverityOrder", "Export Full", new Vector2(100, -18), () =>
            {
                exporter.ExportToDiscordMDByBeats(plugin.currentMapsetReview);
            }, 64, 25, 9, "Copy to clipboard in full format");

            UIHelper.AddButton(_infoMenu.transform, "ExportAMDiscordSeverityOrder", "Export Data", new Vector2(100, -45), () =>
            {
                exporter.ExportToDiscordMDShort(plugin.currentMapsetReview, false, true);
            }, 64, 25, 9, "Copy to clipboard in compact format, data-only");

            UIHelper.AddButton(_infoMenu.transform, "ExportAMDiscordBeatOrder", "Export Compact", new Vector2(164, -18), () =>
            {
                exporter.ExportToDiscordMDShort(plugin.currentMapsetReview, false);
            }, 64, 25, 9, "Copy to clipboard in compact format");

            UIHelper.AddButton(_infoMenu.transform, "ExportAMDiscordManualBeatOrder", "Export Compact (Manual)", new Vector2(164, -45), () =>
            {
                exporter.ExportToDiscordMDShort(plugin.currentMapsetReview, true);
            }, 64, 25, 10, "Copy to clipboard in compact format, ignore autogenerated comments");
