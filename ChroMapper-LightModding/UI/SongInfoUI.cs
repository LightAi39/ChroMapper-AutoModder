...
namespace ChroMapper_LightModding.UI
{
    internal class SongInfoUI
    {
        ...
        public void AddInfoMenu(Transform parent)
        {
            ...
            UIHelper.AddButton(_infoMenu.transform, "ExportAMDiscordBeatOrder", "Export (Compact)", new Vector2(100, -18), () =>
            {
                exporter.ExportToDiscordMDShort(plugin.currentMapsetReview, false);
            }, 64, 25, 9, "Copy to clipboard in compact format");

            UIHelper.AddButton(_infoMenu.transform, "ExportAMDiscordSeverityOrder", "Export (Full)", new Vector2(164, -18), () =>
            {
                exporter.ExportToDiscordMDByBeats(plugin.currentMapsetReview);
            }, 64, 25, 9, "Copy to clipboard in full format");

            UIHelper.AddButton(_infoMenu.transform, "ExportAMDiscordManualBeatOrder", "Export (Compact + Manual-Only) ", new Vector2(100, -45), () =>
            {
