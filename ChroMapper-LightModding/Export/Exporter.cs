...
namespace ChroMapper_LightModding.Export
{
    internal class Exporter
    {
        ...
        public void ExportToDiscordMDShort(MapsetReview review, bool manualOnly, bool dataOnly = false)
        {
            string text = "";

            if (!dataOnly)
            {
                text = string.Join(", ", Enum.GetValues(typeof(CommentTypesEnum))
                .Cast<CommentTypesEnum>()
                .Where(t => t != CommentTypesEnum.Data && review.DifficultyReviews.Any(r => r.Comments.Any(c => c.Type == t && ((manualOnly && !c.IsAutogenerated) || (!manualOnly)))))
                .Select(t => $"({CommentTypeShortening(t)}) = {Exporter.CommentTypeName(t)}"));

                // Erase clipboard if there's nothing to copy
            ...
            foreach (var diffReview in review.DifficultyReviews)
            {
                // Skip if there's no data to write down
                if (dataOnly && !diffReview.Comments.Any(c => c.Type == CommentTypesEnum.Data)) continue;

                // Skip difficulty if there's no comment to write down
                if (string.IsNullOrEmpty(diffReview.OverallComment) && 
                    !diffReview.Comments.Any(c => c.Type != CommentTypesEnum.Data && 
                    ((manualOnly && !c.IsAutogenerated) || (!manualOnly && c.IsAutogenerated)))) continue;

                text += $"{diffReview.Difficulty}:\n";

                foreach (var comment in diffReview.Comments)
                {
                    if (manualOnly && comment.IsAutogenerated) continue;

                    if (dataOnly && comment.Type == CommentTypesEnum.Data)
                    {
                        text += $"{string.Join(", ", comment.Objects.ConvertAll(p => p.ToString()).Distinct())} - {comment.Message}\n";
                    }
                    else if (!dataOnly && comment.Type != CommentTypesEnum.Data)
                    {
                ...
                if (!dataOnly && !string.IsNullOrEmpty(diffReview.OverallComment))
                {
                    text += $"Overall feedback:\n{diffReview.OverallComment}";
                }
            }
