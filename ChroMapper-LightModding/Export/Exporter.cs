...
namespace ChroMapper_LightModding.Export
{
    internal class Exporter
    {
        ...
        public void ExportToDiscordMDShort(MapsetReview review, bool manualOnly)
        {
            string text = string.Join(", ", Enum.GetValues(typeof(CommentTypesEnum))
                .Cast<CommentTypesEnum>()
                .Where(t => t != CommentTypesEnum.Data && review.DifficultyReviews.Any(r => r.Comments.Any(c => c.Type == t && ((manualOnly && !c.IsAutogenerated) || (!manualOnly && c.IsAutogenerated)))))
            ...
            foreach (var diffReview in review.DifficultyReviews)
            {
                ...
                foreach (var comment in diffReview.Comments)
                {
                    if (comment.Type == CommentTypesEnum.Data || (manualOnly && comment.IsAutogenerated)) continue;
