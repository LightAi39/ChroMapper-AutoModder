    ...
    internal static class Obstacle
    {
        ...
        public static CritResult Check(List<Note> notes, List<Wall> walls, List<Bomb> bombs)
        {
            ...
            foreach (var w in walls)
            {
                ...
                if (w.y <= 1 && w.Height > 1 && ((w.x + w.Width == 2 && walls.Exists(wa => wa != w && wa.y <= 1 && wa.Height > 0 && wa.x + wa.Width >= 3 && wa.Beats <= w.Beats + w.DurationInBeats && wa.Beats + wa.DurationInBeats >= w.Beats)) ||
                    (w.x + w.Width == 3 && walls.Exists(wa => wa != w && wa.y <= 1 && wa.Height > 0 && wa.x + wa.Width >= 2 && wa.Beats <= w.Beats + w.DurationInBeats && wa.Beats + wa.DurationInBeats >= w.Beats))))
                {
                    CheckResults.Instance.AddResult(new CheckResult()
                    {
                        Characteristic = CriteriaCheckManager.Characteristic,
