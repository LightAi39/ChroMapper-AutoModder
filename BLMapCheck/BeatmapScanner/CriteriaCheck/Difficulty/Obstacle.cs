...
namespace BLMapCheck.BeatmapScanner.CriteriaCheck.Difficulty
{
    internal static class Obstacle
    {
        ...
        public static CritResult Check(List<Note> notes, List<Wall> walls, List<Bomb> bombs)
        {
            ...
            {
                CheckResults.Instance.CreateDiffResult("Visibility", Severity.Warning, "Wall", "Reduced visibility due to wall", new(), new() { note });
            ...
            foreach (var w in middleWall)
            {
                ...
                {
                    CheckResults.Instance.CreateDiffResult("Hidden", Severity.Error, "Wall", "Notes cannot be hidden behind walls", new(), new() { n });
                ...
                {
                    CheckResults.Instance.CreateDiffResult("Hidden", Severity.Error, "Wall", "Bombs cannot be hidden behind walls", new(), new() { b });
                    criteria = CritResult.Fail;
                }
            }
            ...
            foreach (var w in walls)
            {
                ...
                {
                    CheckResults.Instance.CreateDiffResult("Forced Movement", Severity.Error, "Wall", "Walls cannot force the player to move into the outer lanes", new(), new() { w });
                    criteria = CritResult.Fail;
                }
                ...
                {
                    CheckResults.Instance.CreateDiffResult("Wall Size", Severity.Error, "Wall", "Walls must have positive width, height and duration", new(), new() { w });
                    criteria = CritResult.Fail;
                }
                ...
                {
                    CheckResults.Instance.CreateDiffResult("Wall Length", Severity.Error, "Wall", "Walls cannot be shorter than 13.8ms", 
            ...
            {
                CheckResults.Instance.CreateDiffResult("Wall", Severity.Passed, "Wall", "No issue with hidden objects, movement, wall size and duration detected");
            }
            ...
            for (int i = walls.Count - 1; i >= 0; i--)
            {
                ...
                if (dodge == 1) // Ignore non-dodge walls
                {
                    ...
                    {
                        CheckResults.Instance.CreateDiffResult("Wall Dodge", Severity.Error, "Wall", "Dodge walls must not force the players head to move more than " + Instance.MaximumDodgeWallPerSecond.ToString() + " times per second",
                    ...
                    {
                        CheckResults.Instance.CreateDiffResult("Wall Dodge", Severity.Warning, "Wall", "Dodge walls that force the players head to move more than " + Instance.SubjectiveDodgeWallPerSecond.ToString() + " per second need justification",
            ...
            if (!issue)
            {
                CheckResults.Instance.CreateDiffResult("Wall Dodge", Severity.Passed, "Wall", "No issue with dodge wall found");
            }
